...
def delete_file(filename: str):
    testfile_path = os.path.join(
        os.path.dirname(__file__), f"../gothonweb/sessions/{filename}.txt"
    )
...
class TestApp(unittest.TestCase):
    ...
    def test_app3(self):
        with app.test_client() as web:
            ...
            rv = web.get("/game", follow_redirects=True)
            self.assertIn(
                b"Gothons of Planet Percal #25 have invaded your ship ", rv.data
            )
            data = {"action": "dodge"}
    ...
    def test_app6(self):
        with app.test_client() as web:
            ...
            rv = web.post("/game", follow_redirects=True, data=data)
            self.assertIn(
                b"The container clicks open and the seal breaks, letting gas out.",
                rv.data,
            )
    ...
    def test_load_session1(self):
        with app.test_client() as web:
            ...
            file_exists = exists(
                f"{gothonweb_path}/gothonweb/gothonweb/sessions/new_user_gothonweb.txt"
            )
            assert file_exists
